...
//
//  Created by ABRAR ALQARNI on 21/03/1443 AH.
//
import UIKit
import MapKit

class MapViewControlle: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
  ...
  override func loadView() {
    ...
    segmentedControl.addTarget(self, action: #selector(MapViewControlle.mapTypeChanged(_:)), for: .valueChanged)
  ...
  func initLocalizationButton(_ anyView: UIView!){
    ...
    localizationButton.addTarget(self, action: #selector(MapViewControlle.showLocalization(sender:)), for: .touchUpInside)
  }
  ...
  func mapView(_ mapView: MKMapView, didUpdate userLocation: MKUserLocation) {
    ...
    let zoomedInCurrentLocation = MKCoordinateRegion(center: userLocation.coordinate, latitudinalMeters: 28.3834707, longitudinalMeters: 36.4830335)
